...
class NGL_Block_Metadata extends NGL_Abstract_Block {
	...
	public function tableize( $content, $post_id = 0 ) {
		...
		$replace = '.ngl-table-metadata span';
		foreach( $output->find( $replace ) as $key => $element ) {
		...
		foreach( $output->find( $replace ) as $key => $element ) {
			...
			$element->innertext = '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="ngl-table-metadata" data-date-format="' . $format . '"><tr><td valign="middle" align="' . $align . '">' . $element->innertext . '</td></tr></table>';
		}
